...
static void remocon_list_selected_callback(void *data, Evas_Object *obj, void *event_info);
		...
		conv_error_e error, conv_payload_h payload_h, void* user_data)
{
	...
	if (!(strcmp(result_type, "onStart"))) { // 1. Service started
		...
		// Don't do it when accelerometer was set on pause or restarted
...
discovery_cb(conv_device_h device_h, conv_discovery_result_e result, void* user_data)
{
	...
	case CONV_DISCOVERY_RESULT_SUCCESS: {
		...
		if (has_rss) {
			...
			elm_list_go(__g_ad->remocon_list); // Device provides RSS - adding to the list
...
/*
 * The page with Selecting Remote Control UI ===================================
 */
...
static Eina_Bool page_gameplay_pop_callback(void *data, Elm_Object_Item *it)
{
	...
	// TODO stop and destroy the services
	ui_app_exit();
	return EINA_FALSE;
}
...
app_create(void *data)
{
	...
	if (ret != CONV_ERROR_NONE) {
		...
		// No remote control: Go to the actual game page. (This may be a case of emulator.)
		show_gameplay_page();
	} else {
		dlog_print(DLOG_INFO, LOG_TAG, "Starting Discovery...");
		ret = conv_discovery_start(ad->conv_h, 15, discovery_cb, NULL);
		if (ret != CONV_ERROR_NONE) {
